...
![RabbitMQ-00000055](RabbitMQ-延迟队列(7)/RabbitMQ-00000055.png)

## [#](https://oddfar.github.io/notes/pages/0a0086/#rabbitmq-中的-ttl)RabbitMQ 中的 TTL
...
![RabbitMQ-00000057](RabbitMQ-延迟队列(7)/RabbitMQ-00000057.png)

- **消息设置TTL**

是针对每条消息设置TTL

![RabbitMQ-00000056](RabbitMQ-延迟队列(7)/RabbitMQ-00000056.png)
...
## [#](https://oddfar.github.io/notes/pages/0a0086/#整合-springboot)整合 springboot

前一小节我们介绍了死信队列，刚刚又介绍了 TTL，至此利用 RabbitMQ 实现延时队列的两大要素已经集齐，接下来只需要将它们进行融合，再加入一点点调味料，延时队列就可以新鲜出炉了。想想看，延时队列，不就是想要消息延迟多久被处理吗，TTL 则刚好能让消息在延迟多久之后成为死信，另一方面， 成为死信的消息都会被投递到死信队列里，这样只需要消费者一直消费死信队列里的消息就完事了，因为里面的消息都是希望被立即处理的消息。

1、创建一个空项目：

![RabbitMQ-00000058](RabbitMQ-延迟队列(7)/RabbitMQ-00000058.png)
...
## [#](https://oddfar.github.io/notes/pages/0a0086/#队列-ttl)队列 TTL

- **代码架构图**

创建两个队列 QA 和 QB，两者队列 TTL 分别设置为 10S 和 40S，然后在创建一个交换机 X 和死信交 换机 Y，它们的类型都是direct，创建一个死信队列 QD，它们的绑定关系如下：

![RabbitMQ-00000060](RabbitMQ-延迟队列(7)/RabbitMQ-00000060.png)
...
发起一个请求 [http://localhost:8080/ttl/sendMsg/嘻嘻嘻(opens new window)](http://localhost:8080/ttl/sendMsg/嘻嘻嘻)

![image-20210628162017168](RabbitMQ-延迟队列(7)/image-20210628162017168.png)
...
## [#](https://oddfar.github.io/notes/pages/0a0086/#延时队列ttl优化)延时队列TTL优化

在这里新增了一个队列 QC,绑定关系如下,该队列不设置TTL 时间

![RabbitMQ-00000062](RabbitMQ-延迟队列(7)/RabbitMQ-00000062.png)
...
[http://localhost:8080/ttl/sendExpirationMsg/你好1/20000(opens new window)](http://localhost:8080/ttl/sendExpirationMsg/你好1/20000)

[http://localhost:8080/ttl/sendExpirationMsg/你好2/2000(opens new window)](http://localhost:8080/ttl/sendExpirationMsg/你好2/2000)

![RabbitMQ-00000063](RabbitMQ-延迟队列(7)/RabbitMQ-00000063.png)
...
## [#](https://oddfar.github.io/notes/pages/0a0086/#rabbitmq-插件实现延迟队列)Rabbitmq 插件实现延迟队列
...
![RabbitMQ-00000065](RabbitMQ-延迟队列(7)/RabbitMQ-00000065.png)

代码

在这里新增了一个队列delayed.queue,一个自定义交换机 delayed.exchange，绑定关系如下:

![RabbitMQ-00000066](RabbitMQ-延迟队列(7)/RabbitMQ-00000066.png)
...
- [http://localhost:8080/ttl/sendDelayMsg/hello2/2000(opens new window)](http://localhost:8080/ttl/sendDelayMsg/hello2/2000)

![image-20210628214856678](RabbitMQ-延迟队列(7)/image-20210628214856678.png)

第二个消息被先消费掉了，符合预期

## [#](https://oddfar.github.io/notes/pages/0a0086/#总结)总结
